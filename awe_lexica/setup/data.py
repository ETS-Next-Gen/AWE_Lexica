...
def is_ascii(s):
    '''
    Check if all characters in a string are valid ASCII symbols.

    TODO: Using this introduces bugs and bias for non-English speakers.
    '''
...
def load_text_resource(filename):
    '''
    Load a text resource file from this repository.
    '''
...
def load_json_resource(filename):
    '''
    Load a JSON file from this repository
    '''
...
def load_sylcounts(mobyhyphenator='mhyph.txt'):
    ...
    for line in Lines:
        ...
        for char in line:  # chars:
            ...
            nsyll += 1
        plainline = ''.join(
            [x for x in line if x in string.printable]
        ).replace('\n', '').replace(' ', '_')
...
def to_disk():
    '''
    Process all files and serialize to JSON files on-disk
    '''
